...
test('A-A1 final renderer pass keeps structural fallback plain and upper reconstruction out of 2D collision', () => {
  assert.ok(correctionSource.includes("type ArchStructuralRole = 'pier' | 'upper' | 'lower-panel'"));
  assert.ok(correctionSource.includes("if (role === 'upper' || role === 'pier') return false"));
