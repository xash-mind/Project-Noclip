...
test('Pillar density increases and Ordinary wall density decreases monotonically with continuous Region depth', () => {
  ...
  assert.ok(bins.every((bin) => bin.samples.length >= 10), bins.map((bin) => bin.samples.length).join(','));
  ...
  assert.ok(means[3].columns >= 2.0 && means[3].walls <= means[0].walls * 0.55, JSON.stringify(means));
});

test('Arch dividers preserve complete bay rhythm, route integration and cross-Cell pale continuity', () => {
  ...
  const irregular = specs.filter((spec) => spec.irregular); const irregularRate = irregular.length / specs.length;
  assert.ok(irregularRate > 0 && irregularRate <= ARCH_IRREGULAR_CHANCE * 2.2, `irregular ${(irregularRate * 100).toFixed(2)}%`);
  ...
  assert.ok(specs.every((spec) => spec.bayCount >= 5 && spec.pierCount === spec.bayCount - 1 && spec.terminationCount === 2));
  const routed = specs.filter((spec) => spec.routeBayCount > 0);
  assert.ok(routed.length > specs.length * 0.4, 'too few integrated route bays');
  assert.ok(routed.every((spec) => spec.minimumRouteWidth >= 2.9), `route width ${Math.min(...routed.map((spec) => spec.minimumRouteWidth))}`);

  const cells = window('dev5-arch-seams', 0, 0, 7, tuning);
  ...
  for (let left = 0; left < walls.length; left += 1) for (let right = left + 1; right < walls.length; right += 1) {
    ...
    assert.ok(alongOverlap <= 0.005 || yOverlap <= 0.005, `overlap ${a.id} / ${b.id}`);
  }
  let paleSeams = 0;
  for (let left = 0; left < walls.length; left += 1) for (let right = left + 1; right < walls.length; right += 1) {
    const a = walls[left]; const b = walls[right];
  ...
  assert.ok(paleSeams >= 12, `only ${paleSeams} cross-Cell pale seams`);
});

test('lighting ownership and normal anomaly budgets remain regression-safe', () => {
  ...
  let disorienting = 0; let hallucinations = 0; let groups = 0; let flicker = 0; let off = 0; const total = 2200;
  for (let index = 0; index < total; index += 1) {
    const entry = cell('dev5-long-traversal', index - Math.floor(total / 2), Math.round(Math.sin(index * 0.037) * 9), { ...DEFAULT_TUNING, gateBypass: true });
  ...
  assert.ok(disorienting / total < 0.01); assert.ok(hallucinations / total < 0.003); assert.ok(groups > 1000); assert.ok(flicker / groups < 0.025); assert.ok(off / groups < 0.005);
});