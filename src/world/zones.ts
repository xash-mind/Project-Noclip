import { floorDiv, unitFloat, weightedChoice } from './hash.js';
...
export const ZONE_PROFILES: Record<ZoneId, ZoneProfile> = {
  baseline: { id: 'baseline', label: 'Baseline Lobby', stability: 'disorienting', minimumWorldDay: 0, minimumExposure: 0, wallTint: [0.56, 0.49, 0.25], floorTint: [0.25, 0.22, 0.12], ceilingTint: [0.62, 0.6, 0.45], trimTint: [0.29, 0.25, 0.13], fogDensity: 0.015, lightMultiplier: 1 },
  arch: { id: 'arch', label: 'Arch Rooms', stability: 'semi-stable', minimumWorldDay: 3, minimumExposure: 0.6, wallTint: [0.63, 0.57, 0.35], floorTint: [0.3, 0.25, 0.15], ceilingTint: [0.7, 0.67, 0.52], trimTint: [0.36, 0.31, 0.18], fogDensity: 0.01, lightMultiplier: 1.08 },
  pillar: { id: 'pillar', label: 'Pillar Field', stability: 'disorienting', minimumWorldDay: 3, minimumExposure: 0.6, wallTint: [0.5, 0.46, 0.27], floorTint: [0.2, 0.18, 0.12], ceilingTint: [0.58, 0.56, 0.43], trimTint: [0.25, 0.23, 0.14], fogDensity: 0.024, lightMultiplier: 0.9 },
  ...
  holes: { id: 'holes', label: 'Hole Section', stability: 'terminal', minimumWorldDay: 10, minimumExposure: 2.2, wallTint: [0.42, 0.37, 0.2], floorTint: [0.12, 0.11, 0.075], ceilingTint: [0.48, 0.46, 0.34], trimTint: [0.2, 0.18, 0.1], fogDensity: 0.02, lightMultiplier: 0.72 },
  manila: { id: 'manila', label: 'Manila Room', stability: 'rendezvous', minimumWorldDay: 1, minimumExposure: 0.25, wallTint: [0.68, 0.6, 0.42], floorTint: [0.32, 0.28, 0.21], ceilingTint: [0.73, 0.7, 0.61], trimTint: [0.37, 0.27, 0.17], fogDensity: 0.003, lightMultiplier: 1.22 },
  'exit-threshold': { id: 'exit-threshold', label: 'Exit Threshold', stability: 'semi-stable', minimumWorldDay: 3, minimumExposure: 0.8, wallTint: [0.48, 0.43, 0.24], floorTint: [0.19, 0.17, 0.1], ceilingTint: [0.54, 0.52, 0.41], trimTint: [0.25, 0.22, 0.12], fogDensity: 0.014, lightMultiplier: 0.84 }
};
...
export function chooseZone(seed: string, cellX: number, cellZ: number, worldDay: number, exposure: number, tuning: WorldTuning): ZoneId {
  ...
  const manilaCell = cellX === 8 && cellZ === -6;
  if (manilaCell && isZoneUnlocked('manila', worldDay, exposure, tuning.gateBypass)) return 'manila';
