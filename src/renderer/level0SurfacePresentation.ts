...
function archStructuralMaterial(cache: SurfacePresentationCache, role: 'pier'|'upper'|'lower-panel'): pc.StandardMaterial {
  ...
  const color = materialColor(ARCH_TARGET, keyName, fallback); const gloss = materialNumber(ARCH_TARGET, 'gloss', 0.07);
  return material(cache, `arch-structural:${role}:${color.join(',')}:${gloss}`, () => { const result = makeMaterial(color); result.gloss = gloss; result.update(); return result; });
}

function wallMaterial(cache: SurfacePresentationCache, descriptor: CellDescriptor, wall: WallSpec): pc.StandardMaterial {
  const structural = archStructuralRole(wall); if (structural) return archStructuralMaterial(cache, structural);
...
function setSurfaceMaterial(root: pc.Entity, kind: 'floor'|'ceiling', value: pc.StandardMaterial): void {
  if (kind === 'ceiling') { setMaterial(entityByName(root, 'ceiling'), value); return; }
