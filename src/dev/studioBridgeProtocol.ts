...
export type StudioBridgeCommandType = 'select-target'|'focus-target'|'isolate-target'|'locate-target'|'spawn-showcase'|'preview-parameters'|'preview-assets'|'preview-binding'|'refresh-presentation'|'clear-preview'|'clear-all-previews';
export interface StudioBridgeCommand { id:number; type:StudioBridgeCommandType; targetId?:string; payload?:{parameters?:Readonly<Record<string,PresentationValue>>;assetSlots?:Readonly<Record<string,string>>;representationId?:string}; }
