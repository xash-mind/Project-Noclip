...
function validateAssetPreview(targetId,patch){const definition=definitionForTarget(targetId);if(!definition)throw new Error(`Unknown Studio target ${targetId}`);const slots=new Map((definition.assetSlots??[]).map((slot)=>[slot.key,slot]));const assets=assetLibrary(state);for(const[key,requested]of Object.entries(patch??{})){const slot=slots.get(key);if(!slot?.editable)throw new Error(`${key} is not an editable Asset slot for ${targetId}`);if(typeof requested!=='string'||!requested)throw new Error(`${slot.label} temporary preview requires a concrete compatible Asset; optional unbinding remains a Save-to-Project operation`);const asset=assets.find((item)=>item.id===requested);if(!asset)throw new Error(`Asset ${requested} is not defined in NAL`);if(asset.type!==slot.assetType||asset.profile!==slot.profile||!slot.roles.includes(asset.role))throw new Error(`Asset ${requested} is not compatible with ${slot.label}`);if(asset.runtimeStatus!=='ready')throw new Error(`Asset ${requested} is not runtime-ready; rebuild NAL first`);}}
