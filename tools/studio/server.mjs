...
import { assetLibrary, canonicalContext, canonicalProfiles, canonicalRegistry, canonicalTargets, commandsAfter, createStudioState, focusBridgeTarget, importAsset, latestBridgeState, listReceipts, projectStatus, queueBridgeCommand, readReceipt, recordBridgeState, revertStudioChange, runValidationAction, saveStructuredPresentationChange, studioDiff } from './server-core.mjs';
...
for(const addresses of Object.values(networkInterfaces())){
  for(const address of addresses??[]){
    if(address.family==='IPv4'&&!address.internal)LOCAL_BRIDGE_HOSTS.add(address.address);
  }
}
...
function isDevelopmentBridgeOrigin(origin){
  if(!origin)return false;
  try{
    const parsed=new URL(origin);
    return (parsed.protocol==='http:'||parsed.protocol==='https:')&&LOCAL_BRIDGE_HOSTS.has(parsed.hostname);
  }catch{return false;}
}
function bridgeCors(req,path){
  const origin=req.headers.origin;
  if(!path.startsWith('/api/bridge/')||!isDevelopmentBridgeOrigin(origin))return{};
  const headers={
    'Access-Control-Allow-Origin':origin,
    'Access-Control-Allow-Headers':'Content-Type, X-Noclip-Studio-Token',
...
const server=createServer(async(req,res)=>{
  ...
  const headers=bridgeCors(req,url.pathname);
  if(req.method==='OPTIONS'){
    if(!url.pathname.startsWith('/api/bridge/')||!isDevelopmentBridgeOrigin(req.headers.origin))return json(res,403,{error:'Studio bridge accepts only this machine\'s local development origins'});
  ...
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/api/save'){const result=saveStructuredPresentationChange(state,await body(req));queueBridgeCommand(state,{type:'clear-preview',targetId:result.receipt.semanticTarget.semanticTargetId});queueBridgeCommand(state,{type:'refresh-presentation',targetId:result.receipt.semanticTarget.semanticTargetId});return json(res,200,result);}
    if(req.method==='POST'&&url.pathname==='/api/revert'){const input=await body(req),result=revertStudioChange(state,input.receiptId);queueBridgeCommand(state,{type:'clear-all-previews'});queueBridgeCommand(state,{type:'refresh-presentation'});return json(res,200,result);}
    ...
    const requestPath=url.pathname==='/'?'/index.html':url.pathname,path=safeStatic(CLIENT_ROOT,requestPath);
    if(!existsSync(path))return text(res,404,'Noclip Studio resource not found');
    const bytes=readFileSync(path);res.writeHead(200,{'Content-Type':mime(path),'Content-Length':bytes.length,'Cache-Control':'no-store','X-Content-Type-Options':'nosniff','Content-Security-Policy':"default-src 'self'; img-src 'self' data:; media-src 'self' data:; style-src 'self'; script-src 'self'; connect-src 'self'"});res.end(bytes);
