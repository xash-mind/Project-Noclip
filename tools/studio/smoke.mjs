...
try {
  ...
  if (!wallpaperTarget?.structuredEditable) throw new Error('Studio bootstrap did not expose M-W1 as structured editable');
  ...
  const bridge = await fetch('http://127.0.0.1:4311/api/bridge/state', { method: 'POST', headers: { Origin: origin, 'Content-Type': 'application/json', 'X-Noclip-Studio-Token': token }, body: JSON.stringify({ clientId: 'studio-smoke-client' }) });
  ...
  console.log(`[Studio smoke] bootstrap, M-W1 authoring context and same-host LAN bridge PASS (${origin})`);
