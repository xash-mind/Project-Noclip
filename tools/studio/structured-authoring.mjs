...
const SOURCES = Object.freeze([
  { source: 'src/presentation/definitions/level0-features.json', generated: 'src/presentation/generatedLevel0FeatureDefinitions.ts' },
  { source: 'src/presentation/definitions/level0-materials.json', generated: 'src/presentation/generatedLevel0MaterialDefinitions.ts' }
]);
...
function diffFor(root, paths) { return paths.length ? git(root, ['diff', '--no-ext-diff', '--', ...paths], { allowFailure: true }).stdout : ''; }
function locateTarget(root, targetId) {
  for (const entry of SOURCES) {
    const parsed = readJson(resolve(root, entry.source));
    const binding = parsed.bindings.find((item) => item.semanticTargetId === targetId);
    if (!binding) continue;
    const definition = parsed.representations.find((item) => item.id === binding.representationId);
    if (!definition) throw new Error(`Target ${targetId} binding points to missing Representation ${binding.representationId}`);
    return { entry, parsed, binding, definition };
  }
...
function validateAssetSlotPatch(root, definition, patch) {
  ...
  const sourceAssets = assetDefinitions(root), runtime = runtimeAssets(root);
...
function syncAssetIds(definition) {
  if (!definition.assetSlots) return;
  definition.assetIds = [...new Set(definition.assetSlots.flatMap((slot) => slot.assetId ? [slot.assetId] : []))];
}
...
function storeReceipt(root, envelope) { atomicWrite(receiptPath(root, envelope.id), `${JSON.stringify(envelope, null, 2)}\n`); }
function runValidation(root, targetId) {
  ...
  return {
    ...
    saveCompatibility: passed ? 'PASS' : 'FAIL', warnings: []
  };
}
function assertStudioOwnedOrClean(state, paths) {
  for (const path of paths) if (state.initialDirty?.has(path) && !state.studioTouched?.has(path)) throw new Error(`Studio will not overwrite a pre-existing worktree change: ${path}`);
}

export function isStructuredSourceTarget(root, targetId) { try { locateTarget(root, targetId); return true; } catch { return false; } }

export function saveStructuredSourceChange(state, input) {
  const root = state.root, branch = activeBranch(root);
  const { entry, parsed } = locateTarget(root, input.targetId);
  const sourceRel = entry.source, generatedRel = entry.generated;
  assertStudioOwnedOrClean(state, [sourceRel, generatedRel]);
  ...
  const source = structuredClone(parsed), beforeSource = structuredClone(parsed);
  ...
  validateParameterPatch(liveDefinition, parameterPatch); validateAssetSlotPatch(root, liveDefinition, assetSlotPatch);
  ...
  const beforeAssetIds = [...(liveDefinition.assetIds ?? [])];
  ...
  for (const [key, value] of Object.entries(assetSlotPatch)) {
    ...
    if (!slot) continue;
    if (value === '' && slot.optional) delete slot.assetId; else slot.assetId = value;
  }
  syncAssetIds(liveDefinition);
  ...
  if (input.representationId && input.representationId !== representationBefore) {
    ...
    liveBinding.representationId = input.representationId; representationAfter = input.representationId;
  }
  if (JSON.stringify(source) === JSON.stringify(beforeSource)) throw new Error('Save to Project has no structured change to persist');
  try { atomicWrite(sourcePath, `${JSON.stringify(source, null, 2)}\n`); buildPresentation(root); compileStudio(root); }
  catch (error) { atomicWrite(sourcePath, beforeFile); if (beforeGenerated) atomicWrite(generatedPath, beforeGenerated); throw error; }
  state.studioTouched.add(sourceRel); state.studioTouched.add(generatedRel);
  ...
  const valueSummary = summary(beforeValues, afterValues), assetSummary = summary(beforeAssetSlots, afterAssetSlots);
  const diff = diffFor(root, [sourceRel, generatedRel]);
  const context = canonicalCall(root, 'context', { targetId: input.targetId, options: { branchOrRef: branch, userObservation: input.observation, requestedChange: input.requestedChange }, mode: 'CHANGE' }).context;
  const change = {
    timestamp: new Date().toISOString(), mode: Object.keys(assetSlotPatch).length ? 'asset-replacement' : (representationAfter !== representationBefore ? 'representation-rebind' : 'structured-project-change'),
    beforeValues, afterValues, representationBefore, representationAfter, assetIdsBefore: beforeAssetIds, assetIdsAfter: liveDefinition.assetIds ?? [],
    filesChanged: [sourceRel, generatedRel], generatedFilesChanged: [generatedRel], sourceDefinitionsChanged: [sourceRel], activePreviewState: parameterPatch,
    persisted: true, validation: validation.validations, targetedTests: validation.targetedTests, deterministicIdentity: validation.deterministicIdentity, saveCompatibility: validation.saveCompatibility, warnings: validation.warnings,
    diffSummary: [valueSummary, assetSummary].filter((item) => item !== 'No parameter value changed').join('; ') || 'Structured representation changed',
    actor: { identity: 'local-developer', tool: 'Noclip Studio' }, revert: { kind: 'values', reference: 'Studio targeted structured revert' }
  };
  const canonical = canonicalCall(root, 'receipt', { context, change });
  const id = `${Date.now()}-${input.targetId.replace(/[^a-z0-9_-]+/gi, '-')}`;
  storeReceipt(root, { id, createdAt: new Date().toISOString(), receipt: canonical.receipt, human: canonical.human, json: canonical.json, diff, authoringKind: 'structured-source-v2', localRevert: { files: [
...
export function canRevertStructuredReceipt(root, id) {
  const path = receiptPath(root, id); if (!existsSync(path)) return false;
  try { return readJson(path).authoringKind === 'structured-source-v2'; } catch { return false; }
}
export function revertStructuredSourceChange(state, id) {
  const root = state.root; activeBranch(root); const path = receiptPath(root, id);
  ...
  for (const file of files) { const absolute = allowedStructuredPath(root, file.path); if (!existsSync(absolute) || hash(readFileSync(absolute)) !== file.afterHash) throw new Error(`Targeted revert refused: ${file.path} changed after this Studio operation. Review the diff manually.`); }
  for (const file of files) { atomicWrite(allowedStructuredPath(root, file.path), file.beforeContent); state.studioTouched.add(file.path); }
  compileStudio(root);
